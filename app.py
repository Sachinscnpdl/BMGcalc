...
# Custom CSS (unchanged, but kept for completeness)
st.markdown("""
<style>
    /* Main Styles */
    .main-header {
...
with col1:
    ...
    if mode == "Single Alloy":
        # Existing single alloy UI
        
        # If predictions exist, show a compact composition view
        if st.session_state.predictions is not None:
            # Show current composition in a compact card
            ...
            # Edit button to modify composition
            if st.button("✏️ Edit Composition", use_container_width=True, type="secondary"):
                st.session_state.predictions = None
                st.session_state.show_periodic_table = True
                st.rerun()
        
        # If no predictions, show the full composition setup
        else:
            ...
            # Manual Composition Input - Toggle button outside card
            st.markdown('<div style="margin-bottom: 0.5rem;">', unsafe_allow_html=True)
            if st.button("📝 Manual Input", key="toggle_manual", use_container_width=True, 
                         type="secondary" if not st.session_state.show_manual_input else "primary"):
                st.session_state.show_manual_input = not st.session_state.show_manual_input
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
            
            # Manual input card - only shown when toggled on
            ...
            # Selected Elements Display (only if elements selected)
            ...
            # Composition Input (only if elements selected)
            if st.session_state.selected_elements:
                ...
                if 'Alloys' not in df_preview.columns:
                    ...
                    if st.button("🚀 Run Batch Prediction", use_container_width=True, type="primary"):
                        ...
                        with st.spinner("Processing batch... This may take a while."):
                            # Reset file pointer because we already read it
...
with col2:
    ...
    if mode == "Single Alloy":
        if st.session_state.predictions is not None:
            ...
            # Key Metrics (now only two cards, phase gets more space)
            metric_cols = st.columns([1, 2])  # ΔT takes 1, Phase takes 2
            with metric_cols[0]:
                st.markdown(f"""
        ...
        if st.session_state.batch_results is not None:
            ...
            <div class="glass-card" style="text-align: center; padding: 2rem 1.5rem;">
                ...
                <div style="margin-top: 1.5rem; color: #94A3B8; font-size: 0.8rem;">
                    <div style="font-style: italic; color: #64748B;">
                        Example: Cu50Zr50, Fe40Ni40P14B6, etc.
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
