...
<style>
/* (your CSS unchanged) */
.main-header { background: linear-gradient(90deg, #0F172A 0%, #1E293B 100%); padding: 1.5rem 2rem; border-radius: 12px; margin-bottom: 2rem; text-align: center; color: white; font-size: 2rem; font-weight: 700; letter-spacing: -0.02em; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3); border: 1px solid rgba(0, 180, 219, 0.2); font-family: 'Space Grotesk', sans-serif; position: relative; }
.reset-btn-container { position: absolute; top: 1rem; right: 2rem; }
.main-container { max-width: 100% !important; width: 100% !important; padding: 0 1rem; }
.glass-card { background: rgba(30, 41, 59, 0.8); border: 1px solid rgba(0, 180, 219, 0.2); border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.2); backdrop-filter: blur(10px); transition: all 0.3s ease; }
.section-title { color: #00B4DB; font-size: 1.1rem; font-weight: 700; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem; font-family: 'Space Grotesk', sans-serif; }
.element-tag { background: linear-gradient(90deg, #00B4DB, #0083B0); color: white; padding: 0.3rem 0.6rem; border-radius: 6px; font-size: 0.85rem; font-weight: 600; display: inline-block; margin: 0.2rem; box-shadow: 0 2px 8px rgba(0, 180, 219, 0.3); transition: all 0.2s ease; }
.element-tag:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(0, 180, 219, 0.4); }
.metric-card { background: linear-gradient(135deg, rgba(0, 180, 219, 0.1), rgba(0, 131, 176, 0.1)); border: 1px solid rgba(0, 180, 219, 0.3); border-radius: 10px; padding: 1rem; text-align: center; transition: all 0.3s ease; height: 100%; display: flex; flex-direction: column; justify-content: center; }
.metric-value { color: #00B4DB; font-size: 2rem; font-weight: 800; font-family: 'Space Grotesk', sans-serif; line-height: 1.2; text-shadow: 0 0 8px rgba(0, 180, 219, 0.3); }
.property-row { display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 0; border-bottom: 1px solid rgba(0, 180, 219, 0.1); transition: all 0.2s ease; }
.compact-composition { background: rgba(30, 41, 59, 0.5); border: 1px solid rgba(0, 180, 219, 0.2); border-radius: 8px; padding: 1rem; margin-bottom: 1rem; }
.composition-string { font-family: 'Space Grotesk', monospace; font-size: 1.2rem; font-weight: 600; color: #00B4DB; letter-spacing: 0.5px; }
.gauge-container { background: rgba(30, 41, 59, 0.9); border: 1px solid rgba(0, 180, 219, 0.3); border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); }
.stButton > button { background: linear-gradient(90deg, #00B4DB, #0083B0); color: white; border: none; border-radius: 8px; font-weight: 600; padding: 0.6rem 1.2rem; transition: all 0.3s ease; font-family: 'Inter', sans-serif; }
.examples-box { background: #1F2A3A; border: 1px solid #2D3A4A; border-radius: 6px; padding: 0.6rem; margin: 0.5rem 0; font-size: 0.75rem; color: #E0E0E0; line-height: 1.5; }
.stTextInput > div > div > input { background: rgba(30, 41, 59, 0.7) !important; border: 1px solid rgba(0, 180, 219, 0.3) !important; color: white !important; border-radius: 6px; padding: 0.5rem 0.75rem !important; }
.error-message { background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 8px; padding: 1rem; color: #F87171; margin: 1rem 0; }
</style>
...
def create_composition_pie(elements, fractions):
    ...
    fig = go.Figure(data=[go.Pie(labels=elements, values=fractions, hole=.4,
...
def parse_composition_string(comp_str):
    ...
    for match in matches:
        ...
        fractions.append(float(match[1]))
        total += float(match[1])
    if total > 0:
...
def handle_element_change(changed_element, new_value):
    ...
    if changed_element in locked:
        ...
        unlocked = [e for e in all_elements if e not in locked]
        other_unlocked = [e for e in unlocked if e != changed_element]
        if other_unlocked:
            locked_total = sum(st.session_state.element_fractions.get(e, 0) for e in locked)
            current = st.session_state.element_fractions[changed_element]
            max_allowed = 100 - locked_total
            if current > max_allowed:
                st.session_state.element_fractions[changed_element] = max_allowed
                current = max_allowed
            remaining = max(0, 100 - locked_total - current)
...
with col1:
    ...
    mode = st.radio("Select input mode:", options=["Single Alloy", "Batch CSV"], horizontal=True, key="input_mode")

    if mode == "Single Alloy":
        if st.session_state.predictions is not None:
            composition_str = "".join([f"{elem}{int(st.session_state.element_fractions.get(elem,0))}"
            ...
            num_elements = st.select_slider("Number of elements", options=[2,3,4,5,6], value=3, key="num_elements")
            ...
            with show_hide_col2:
                ...
                if st.button(button_label, key="toggle_table"):
                    ...
                    for col_idx, element in enumerate(row):
                        if element:
                            with cols[col_idx]:
                                ...
                                btn_key = f"btn_{safe_key(element)}_{row_idx}_{col_idx}"
                                if st.button(element, key=btn_key, use_container_width=True):
            ...
            if not st.session_state.show_manual_input:
                if st.button("Manual Input", key="toggle_manual"):
            ...
            if st.session_state.show_manual_input:
                ...
                comp_string = st.text_input("Enter composition (e.g., Cu50Zr50 or Cu50Zr25Al25):",
                                           key="composition_string", placeholder="Cu50Zr50",
                ...
                if st.button("Apply & Predict", key="apply_composition", use_container_width=True):
                    if comp_string:
                        ...
                        if elements and fractions:
                            ...
                            st.session_state.element_fractions = {e: f for e,f in zip(elements, fractions)}
                            st.session_state.locked_elements = []
                            st.session_state.show_manual_input = False
                            st.session_state.prediction_error = None
                            composition = "".join([f"{e}{int(st.session_state.element_fractions[e])}" for e in elements])
                ...
                if st.session_state.selected_elements:
                    ...
                    for elem in st.session_state.selected_elements:
                        ...
                        with col_left:
                            ...
                            slider_key = f"slider_{safe_key(elem)}"
                            new_val = st.slider(elem, min_value=0.0, max_value=100.0,
                                                value=float(current_val), step=0.5, key=slider_key,
                                                disabled=is_locked)
                        ...
                        with col_right:
                            ...
                            if st.button(lock_label, key=btn_key, disabled=(not can_lock and not is_locked)):
                                if is_locked:
                    ...
                    total = sum(st.session_state.element_fractions.get(e,0) for e in st.session_state.selected_elements)
                    st.progress(total/100.0, text=f"Total: {total:.1f}%")
                    if abs(total - 100) <= 0.1:
                        st.markdown('<div style="color: #4ADE80; font-weight: 600;">Valid</div>', unsafe_allow_html=True)
                    else:
                        st.markdown('<div style="color: #F87171; font-weight: 600;">Adjusting...</div>', unsafe_allow_html=True)
                        auto_adjust_composition()
                        st.rerun()
                    ...
                    if st.button("Predict Properties", use_container_width=True):
                        composition = "".join([f"{e}{int(st.session_state.element_fractions[e])}" for e in st.session_state.selected_elements])
                        st.session_state.prediction_error = None
                        with st.spinner("Analyzing alloy composition..."):
                            st.session_state.predictions = process_single_alloy(composition)
                            if st.session_state.predictions:
            ...
            # non-manual composition display & sliders (same as above but outside manual card)
            if not st.session_state.show_manual_input and st.session_state.selected_elements:
                ...
                for elem in st.session_state.selected_elements:
                    ...
                    with col_left:
                        is_locked = elem in st.session_state.locked_elements
                        slider_key = f"slider_{safe_key(elem)}"
                        new_val = st.slider(elem, min_value=0.0, max_value=100.0, value=float(current_val),
                                            step=0.5, key=slider_key, disabled=is_locked)
                        if abs(new_val - current_val) > 0.01:
                            handle_element_change(elem, new_val)
                            st.rerun()
                    with col_mid:
        ...
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        uploaded_file = st.file_uploader("Upload a CSV file with an 'Alloys' column", type=['csv'])
        if uploaded_file is not None:
            try:
                ...
                if 'Alloys' not in df_preview.columns:
                    ...
                    if st.button("Run Batch Prediction", use_container_width=True, key="run_batch"):
                        st.session_state.batch_error = None
    ...
    if mode == "Single Alloy":
        if st.session_state.predictions is not None:
            ...
            if not st.session_state.show_periodic_table:
                if st.button("Show Periodic Table", key="show_table_results"):
                    st.session_state.show_periodic_table = True
                    st.rerun()
            st.markdown('<div class="glass-card">', unsafe_allow_html=True)
            ...
            with metric_cols[0]:
                st.markdown(f"<div class='metric-card'><div class='metric-label'>Delta T</div><div class='metric-value'>{pred['Predicted_Tx'] - pred['Predicted_Tg']:.0f}</div><div class='metric-sub'>Tx - Tg (K)</div></div>", unsafe_allow_html=True)
            with metric_cols[1]:
                st.markdown(f"<div class='metric-card'><div class='metric-label'>Phase</div><div class='metric-value phase'>{pred['Predicted_Phase']}</div><div class='metric-sub'>Confidence: {pred['Phase_Confidence']:.1%}</div></div>", unsafe_allow_html=True)
            properties = [("Glass Transition (Tg)", f"{pred['Predicted_Tg']:.1f} K"), ("Crystallization (Tx)", f"{pred['Predicted_Tx']:.1f} K"), ("Liquidus (Tl)", f"{pred['Predicted_Tl']:.1f} K"), ("Critical Diameter", f"{pred['Predicted_Dmax']:.4f} mm"), ("Critical Cooling Rate", f"{pred['Predicted_Rc']:.3f} K/s")]
            for name, value in properties:
                st.markdown(f"<div class='property-row'><div class='property-label'>{name}</div><div class='property-value'>{value}</div></div>", unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            composition = "".join([f"{e}{int(st.session_state.element_fractions.get(e,0))}" for e in st.session_state.selected_elements])
            results_df = pd.DataFrame({'Alloy':[composition],'Phase':[pred['Predicted_Phase']],'Confidence':[pred['Phase_Confidence']],'Tg_K':[pred['Predicted_Tg']],'Tx_K':[pred['Predicted_Tx']],'Tl_K':[pred['Predicted_Tl']],'Delta_T_K':[pred['Predicted_Tx']-pred['Predicted_Tg']],'Dmax_mm':[pred['Predicted_Dmax']],'Rc_Ks':[pred['Predicted_Rc']]})
            ...
            st.markdown(f"<div class='error-message'><div style='font-weight:600; margin-bottom:0.5rem;'>Prediction Failed</div><div style='font-size:0.9rem;'>{st.session_state.prediction_error}</div><div style='margin-top:1rem; font-size:0.85rem; color:#FCA5A5;'>Please check your composition and try again.</div></div>", unsafe_allow_html=True)
            ...
            st.markdown('<div class="glass-card" style="text-align:center; padding:2rem 1.5rem;"><div style="color:#FFFFFF; font-size:1.2rem; font-weight:700;">READY FOR ANALYSIS</div><div style="color:#CBD5E1; font-size:0.9rem; margin-top:0.5rem;">Select elements and set composition</div></div>', unsafe_allow_html=True)
        ...
        if st.session_state.batch_results is not None:
            ...
            st.markdown(f"<div class='error-message'><div style='font-weight:600; margin-bottom:0.5rem;'>Batch Processing Failed</div><div style='font-size:0.9rem;'>{st.session_state.batch_error}</div><div style='margin-top:1rem; font-size:0.85rem; color:#FCA5A5;'>Please check your CSV file and try again.</div></div>", unsafe_allow_html=True)
            if st.button("Clear Error", key="clear_batch_error"):
                st.session_state.batch_error = None
                st.rerun()
        else:
            st.markdown('<div class="section-title">Batch Panel</div>', unsafe_allow_html=True)
            st.markdown('<div class="glass-card" style="text-align:center; padding:2rem 1.5rem;"><div style="color:#FFFFFF; font-size:1.2rem; font-weight:700;">UPLOAD A CSV FILE</div><div style="color:#CBD5E1; font-size:0.9rem; margin-top:0.5rem;">Your file must contain an \"Alloys\" column.</div></div>', unsafe_allow_html=True)

st.markdown('</div>', unsafe_allow_html=True)
st.markdown('<div style="text-align:center; padding:1.5rem; margin-top:2rem; border-top:1px solid rgba(255,255,255,0.1);"><div style="color:#94A3B8; font-size:0.85rem; font-weight:500;">BMGcalc v2.0 • Bulk Metallic Glass Design Platform</div></div>', unsafe_allow_html=True)
