...
# Sanitizer helpers for any dynamic strings (pipeline outputs, compositions)
...
def sanitize_display(s: str) -> str:
    """Remove invisible characters and non-ASCII. Trim whitespace."""
    if s is None:
        return s
    s = str(s)
...
def sanitize_dataframe_strings(df: pd.DataFrame) -> pd.DataFrame:
    """Sanitize all object/string columns in a DataFrame."""
    for col in df.select_dtypes(include=['object']).columns:
        df[col] = df[col].astype(str).apply(sanitize_display)
    return df

# Custom CSS – improved contrast for results and examples (ASCII-only comments)
st.markdown(r"""
<style>
    /* Main Styles */
    .main-header {
    ...
    .glass-card:hover { border-color: rgba(0, 180, 219, 0.4); box-shadow: 0 12px 40px rgba(0, 180, 219, 0.15); }
    ...
    .metric-card:hover { transform: translateY(-3px); box-shadow: 0 8px 25px rgba(0, 180, 219, 0.2); }
    .metric-label { color: #94A3B8; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.5rem; }
    ...
    .metric-value.phase { font-size: 2.5rem; font-weight: 900; }
    .metric-sub { color: #64748B; font-size: 0.7rem; margin-top: 0.3rem; font-weight: 500; }
    ...
    .property-row:hover { background: rgba(0, 180, 219, 0.05); padding-left: 0.5rem; padding-right: 0.5rem; margin: 0 -0.5rem; border-radius: 6px; }
    .property-row:last-child { border-bottom: none; }
    .property-label { color: #A0AEC0; font-size: 0.9rem; font-weight: 500; }
    .property-value { color: #00B4DB; font-size: 0.95rem; font-weight: 700; font-family: 'Space Grotesk', sans-serif; }
    ...
    .stButton > button:hover { background: linear-gradient(90deg, #0083B0, #006994); transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0, 180, 219, 0.3); }
    ...
    .stTextInput > div > div > input:focus { border-color: #00B4DB !important; box-shadow: 0 0 0 2px rgba(0, 180, 219, 0.2) !important; }
...
def process_single_alloy(composition_string):
    ...
    try:
        ...
        # sanitize dataframe strings first
        result_df = sanitize_dataframe_strings(result_df)
        row = result_df.iloc[0]
        pred = {
            'Alloys': sanitize_display(row.get('Alloys', '')),
            'Predicted_Phase': sanitize_display(row.get('Predicted_Phase', '')),
            'Phase_Confidence': float(row.get('Phase_Confidence', 0.0)),
            'Predicted_Tg': float(row.get('Predicted_Tg', 0.0)),
            'Predicted_Tx': float(row.get('Predicted_Tx', 0.0)),
            'Predicted_Tl': float(row.get('Predicted_Tl', 0.0)),
            'Predicted_Dmax': float(row.get('Predicted_Dmax_mm', 0.0)),
            'Predicted_Rc': float(row.get('Predicted_Rc_Ks', 0.0))
        }
        return pred
...
def get_download_link(df, filename="bmg_predictions.csv"):
    ...
    href = f'''
    <a href="data:file/csv;base64,{b64}" download="{sanitize_display(filename)}"
...
with col1:
    ...
    if mode == "Single Alloy":
        ...
        if st.session_state.predictions is not None:
            ...
            composition_str = sanitize_display(composition_str)
            ...
            <div class="compact-composition">
                ...
                <div style="margin-top: 0.5rem; display: flex; flex-wrap: wrap; gap: 0.3rem;">
                    {''.join([f'<span class="element-tag">{sanitize_display(elem)}</span>' for elem in st.session_state.selected_elements])}
                </div>
            </div>
            ''', unsafe_allow_html=True)
        ...
        # No predictions: show full composition setup
                    ...
                    if comp_string:
                        ...
                        if elements and fractions:
                            ...
                            composition = "".join([f"{elem}{int(st.session_state.element_fractions[elem])}"
                                                  for elem in st.session_state.selected_elements])
                            composition = sanitize_display(composition)
            ...
            # Non-manual composition card (same sliders outside manual)
    ...
    if mode == "Single Alloy":
        if st.session_state.predictions is not None:
            ...
            with metric_cols[0]:
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value">{int(pred['Predicted_Tx'] - pred['Predicted_Tg'])}</div>
            ...
            with metric_cols[1]:
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value phase">{sanitize_display(pred.get('Predicted_Phase',''))}</div>
                    <div class="metric-sub">Confidence: {pred.get('Phase_Confidence',0.0):.1%}</div>
                </div>
                """, unsafe_allow_html=True)
            properties = [
            ...
            for name, value in properties:
                ...
                <div class="property-row">
                    <div class="property-label">{sanitize_display(name)}</div>
                    <div class="property-value">{sanitize_display(value)}</div>
                </div>
                ''', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            composition = "".join([f"{elem}{int(st.session_state.element_fractions.get(elem,0))}"
                                 for elem in st.session_state.selected_elements])
            composition = sanitize_display(composition)
            results_df = pd.DataFrame({
                'Alloy': [composition],
                'Phase': [sanitize_display(pred.get('Predicted_Phase',''))],
                'Confidence': [pred.get('Phase_Confidence',0.0)],
            ...
            <div class="error-message">
                ...
                <div style="font-size: 0.9rem;">{sanitize_display(st.session_state.prediction_error)}</div>
            ...
            <div class="glass-card" style="text-align: center; padding: 2rem 1.5rem;">
                <div style="color: #FFFFFF; font-size: 1.2rem; font-weight: 700;">READY FOR ANALYSIS</div>
                <div style="color: #CBD5E1; font-size: 0.9rem; margin-top: 0.5rem;">Select elements and set composition</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if st.session_state.batch_results is not None:
            ...
            <div class="error-message">
                ...
                <div style="font-size: 0.9rem;">{sanitize_display(st.session_state.batch_error)}</div>
                <div style="margin-top: 1rem; font-size: 0.85rem; color: #FCA5A5;">
            ...
            st.markdown("""
            <div class="glass-card" style="text-align: center; padding: 2rem 1.5rem;">
                <div style="color: #FFFFFF; font-size: 1.2rem; font-weight: 700;">UPLOAD A CSV FILE</div>
                <div style="color: #CBD5E1; font-size: 0.9rem; margin-top: 0.5rem;">Your file must contain an "Alloys" column.</div>
            </div>
            """, unsafe_allow_html=True)
